...
connection + manual jog control on the Connection tab is real: it owns a
real am5.gui.worker.MountWorker, unchanged from the ISS tracker, talking
to either a MockMount or real serial hardware -- same event-pump idiom as
am5/gui/app.py's own _pump_events (EVENT_POLL_MS below).
"""
...
class App:
    def __init__(self, root: tk.Tk):
        ...
        self.connection_panel = ConnectionPanel(self.notebook, mount_worker=self.mount_worker)
        ...
        self.reference_panel = AcquisitionPanel(
            self.notebook, role="reference", seed=7, get_star_name=self.target_panel.get_reference_name,
            get_spectrum=self.target_panel.get_reference_spectrum,
        )
        self.target_capture_panel = AcquisitionPanel(
            self.notebook, role="target", seed=11, get_star_name=self.target_panel.get_target_name,
            get_spectrum=self.target_panel.get_target_spectrum_model,
        )
        ...
        self.spectrum_panel = SpectrumPanel(self.notebook, reduction_panel=self.reduction_panel, target_panel=self.target_panel)
        ...
        self._log("Frontend mockup -- mount connection + jog are real (am5.gui.worker.MountWorker), the rest isn't yet.")
